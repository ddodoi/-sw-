...
level = 0
next_level = [["<소년기>-공부", "무언가를 배우고 좋은 성적을 받는 것을 좋아하는군요.", "<소년기>-예체능", "당신의 끼를 발산하는 것을 좋아하는군요."],
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE:
                ...
                if level < 3:
                    level_text(next_level[level][0], next_level[level][1])
                    level += 1
            elif event.key == pygame.K_e:
                if level < 3:
                    level_text(next_level[level][2], next_level[level][3])
                    level += 1
        if level >= 3:
